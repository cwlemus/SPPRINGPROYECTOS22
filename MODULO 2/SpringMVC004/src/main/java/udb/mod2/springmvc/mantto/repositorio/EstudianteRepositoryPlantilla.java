package udb.mod2.springmvc.mantto.repositorio;
...
public class EstudianteRepositoryPlantilla {
	...
	public List<Estudiante> buscarByNombre(String nombre) {
		TypedQuery<Estudiante> consultaJPA = em
				.createQuery("select f from Estudiante f where f.nombre like '%" + nombre + "%'", Estudiante.class);
		return consultaJPA.getResultList();
	}
